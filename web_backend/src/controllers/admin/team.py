...
from src.infrastructure.telemetry.request_context import current_tenant_id, current_user_id
...
from src.controllers.admin.admin_user_helpers import UserUpdateBase, create_hashed_user
...
async def list_team_members(current_user: Any = Depends(require_roles(["ORG_ADMIN", "SUPER_ADMIN", "RECEPTIONIST", "DOCTOR"]))):
    ...
    from src.repositories.tenant_aware_repository import user_repo
...
@router.get("/roles")
async def get_roles():
    from src.infrastructure.auth.permissions import ROLE_PERMISSIONS
